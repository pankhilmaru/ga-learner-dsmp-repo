...
for i in range(len(sample_size)):
    m=[]
    for j in range(1000):
        temp = data.sample(n=sample_size[i])
        m.append(temp['installment'].mean())
...
#Code starts here
yes = data['purpose'].where(data['paid.back.loan']=='Yes').value_counts()

no = data['purpose'].where(data['paid.back.loan']=='No').value_counts()
